...
def annulus_mask(image, center, radius=10., filler=0.0):
    Y, X = np.ogrid[:len(image[:,0]),:len(image[0,:])]
    dist_from_center = np.sqrt((X - 140.)**2 + (Y-140.)**2)
    bin_radius = np.sqrt((center[0] - 145.)**2 + (center[1]-145.)**2)

    dist_from_center[dist_from_center < bin_radius - radius] = 1.0
    dist_from_center[dist_from_center >= bin_radius+radius] = 1.0
    dist_from_center[dist_from_center > 1.1] = filler
    image = dist_from_center
    return image
...
def stellar_polarisation_subtraction(STOKES,stokes_name,MINR=0.0,MAXR=140.0,Bcent_stokes=(0,0)):
    ...
    if Bcent_stokes[0] != 0:
        #maskI = circular_mask(STOKES[0],([int(Bcent_stokes[0]),int(Bcent_stokes[1])]),radius=20,filler=np.nan)
        #print('Binary companion masked at: (x,y) = ',Bcent_stokes)
        maskI = annulus_mask(STOKES[0],([int(Bcent_stokes[0]),int(Bcent_stokes[1])]),radius=10,filler=np.nan)
        print('Binary companion masked at: (x,y) = ',Bcent_stokes)
    else:
    ...
    MEANI = np.nanmedian((STOKES[0]*ratio)[XPIX,YPIX])
    MEANQ = np.nanmedian((STOKES[1]*ratio)[XPIX,YPIX])
    MEANU = np.nanmedian((STOKES[2]*ratio)[XPIX,YPIX])
    FQ = MEANQ/MEANI
    FU = MEANU/MEANI
    ...
    #Save the image map that is used to mask out the disk, binary, and 
    hdr =fits.Header() #create a new header
    hdr['COMMENT'] = "This file is an image of the pixels that were masked."
    hdr['COMMENT'] = "Values of 1.0 are good, values of np.nan are masked."
    hdr['COMMENT'] = "values are masked based on their Q/I and U/I ratio, "
    hdr['COMMENT'] = "if a bright companion is present, and if there is enough flux"
    hdu = fits.PrimaryHDU(ratio,header=hdr)
    hdu.writeto(stokes_name.split('.fits')[0] + '_mask.fits' ,overwrite=True)
    #hdu.close()
...
def qphi_uphi_subplots(directory, title):
    filt = directory.split('-')[-1][0]
    if filt == 'H':
        filt_size = 8.6 #mask size in pixels
    elif filt == 'J':
        filt_size = 6.5 #mask size in pixels
    files = glob.glob(directory + "*_podc_rstokesdc.fits")
    files = sorted(files)
    fig = plt.figure(figsize=(6,3*len(files)))
    fig.suptitle(title,fontsize=14)
    gs1 = gridspec.GridSpec(len(files),2)
    for i in range(0,len(files)):
        hdul = fits.open(files[i])
        data = hdul['sci'].data
        hdul.close()
        VMAX = 1000. # np.nanmax(data[1,:,:])/50
        VMIN = -VMAX
        ax1 =  plt.subplot(gs1[i*2])
        ax2 = plt.subplot(gs1[i*2+1])
        img1 = ax1.imshow(data[1,:,:], origin = 'lower', CMAP='seismic', #vmin= -50, vmax = 50,
        ...
        circ1 = Circle((140,140),radius=filt_size,ls='-',fill=False,color='g',lw=2)
        ax1.add_patch(circ1)
 
        VMAX = 1000. #np.nanmax(data[2,:,:])/100
        VMIN = -VMAX
        img1 = ax2.imshow(data[2,:,:], origin = 'lower', CMAP='seismic', #vmin= -50, vmax = 50,
                   norm=matplotlib.colors.SymLogNorm(linthresh=10.0, 
                   linscale=1.0, vmin=VMIN, vmax=VMAX))
        ax2.set_yticks([])
        ax2.set_yticklabels([])
    ...
    plt.savefig(directory + 'rstokesdc_subplots_' + title + '.png',dpi=150)
...
def QUrot_subplots(directory, ax4,ax5,fig,MINR,MAXR,filt_size):
    ...
    VMAX = 1000. #np.nanmax(data[1,:,:])/50
    VMIN = -1000.
    img1 = ax4.imshow(data[1,:,:], origin = 'lower', CMAP='seismic', #vmin= -50, vmax = 50,
               norm=matplotlib.colors.SymLogNorm(linthresh=10.0, 
               linscale=1.0, vmin=VMIN, vmax=VMAX))
    ...
    circ1 = Circle((140,140),radius=MINR,ls='--',fill=False,color='r')
    circ2 = Circle((140,140),radius=MAXR,ls='--',fill=False,color='r')
    ax4.add_patch(circ1)
    ax4.add_patch(circ2) 
    circ3 = Circle((140,140),radius=filt_size,ls='-',fill=False,color='g',lw=2)
    ax4.add_patch(circ3)
    ...
    circ3 = Circle((140,140),radius=filt_size,ls='-',fill=False,color='g',lw=2)
    ax5.add_patch(circ3)
    ax5.set_xlabel('Combined Uphi image')

def polarization_plots(directory,name,MINR,MAXR):
    ...
    QUrot_subplots(directory, ax4,ax5, fig,MINR,MAXR,filt_size)
