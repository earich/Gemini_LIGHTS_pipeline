...
def createrecipe(INPUT_FILES, INPUT_DIRECTORY, REDUCED_DIRECTORY,
                 QUEUE_DIRECTORY, KEYS=[], RECIPE=0, PRINT_USER_INFO='', RETURN_OUTPUT_DIRECTORY=0, OUTPUT_DIR=''):
    """
    ...
    PRINT_USER_INFO - To bypass printing user info (username, pc name)
                      in the recipe file.
    OUTPUT_DIR - Output directory. If unspecified, one is created.

    Returns:
    OUTPUT_DIR - This output directory will be the input
    ...
    if len(OUTPUT_DIR)<1:
        # If no directory specified by user, create it now:
        OUTPUT_DIR = REDUCED_DIRECTORY + SHORTDATE + '/' +\
